$(document).ready(function() {
	module("pointpicker.js");

	test("parse points from string", function() {
		var points=parsePointPickerFrom("0,1,2,3,4,5");
	...
	test("parse points keep only string item", function() {
		var points=parsePointPickerFrom("0,hello,5");
		equal($.isArray(points),true);
	...
	test("parse points trim item before number conversion", function() {
		var points=parsePointPickerFrom(" 0, 1   , 67");
		equal($.isArray(points),true);
	...
	test("parse points allow decimal values", function() {
		var points=parsePointPickerFrom(" 0, 1.5   , 9.37");
		equal($.isArray(points),true);
		equal(points.length,3);
		equal(points[0], 0);
		equal(points[1], 1.5);
